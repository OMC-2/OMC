    ...
    public void handle(List<ConsumerRecord<String, String>> records, Acknowledgment acknowledgment) {
        ...
        for (int i = 0; i < records.size(); i++) {
            ...
            try {
                ...
                log.info("[OrderShippedConsumer] 수신. topic={}, offset={}, eventId={}", record.topic(), record.offset(), event.get("eventId"));
                events.add(event);
            } catch (Exception e) {
        ...
        for (Map<String, Object> event : events) {
            ...
            if (duplicates.contains(eventId)) {
                log.debug("[OrderShippedConsumer] 중복 이벤트 스킵. eventId={}", eventId);
                continue;
            }
            UUID userId = UUID.fromString(String.valueOf(event.get("userId")));
            UUID orderId = UUID.fromString(String.valueOf(event.get("orderId")));
            toSave.add(Notification.createPending(userId, NotificationType.ORDER_SHIPPED,
                    "배송 시작 알림", "주문하신 상품이 배송을 시작했습니다.", orderId, "ORDER"));
        }
