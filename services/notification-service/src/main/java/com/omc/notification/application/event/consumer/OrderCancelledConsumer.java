    ...
    public void handle(List<ConsumerRecord<String, String>> records, Acknowledgment acknowledgment) {
        ...
        for (int i = 0; i < records.size(); i++) {
            ...
            try {
                ...
                log.info("[OrderCancelledConsumer] 수신. topic={}, offset={}, eventId={}", record.topic(), record.offset(), event.get("eventId"));
                events.add(event);
            } catch (Exception e) {
        ...
        for (Map<String, Object> event : events) {
            ...
            if (duplicates.contains(eventId)) {
                log.debug("[OrderCancelledConsumer] 중복 이벤트 스킵. eventId={}", eventId);
                continue;
            }
            ...
            String content = "주문이 취소되었습니다." + (reason.isBlank() ? "" : " 사유: " + reason);
            toSave.add(Notification.createPending(userId, NotificationType.ORDER_CANCELLED,
                    "주문 취소 알림", content, orderId, "ORDER"));
        }
