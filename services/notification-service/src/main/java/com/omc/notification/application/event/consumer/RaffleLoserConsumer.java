    ...
    public void handle(List<ConsumerRecord<String, String>> records, Acknowledgment acknowledgment) {
        ...
        for (int i = 0; i < records.size(); i++) {
            ...
            try {
                ...
                log.info("[RaffleLoserConsumer] 수신. topic={}, offset={}, eventId={}", record.topic(), record.offset(), event.get("eventId"));
                events.add(event);
            } catch (Exception e) {
        ...
        for (Map<String, Object> event : events) {
            ...
            if (duplicates.contains(eventId)) {
                log.debug("[RaffleLoserConsumer] 중복 이벤트 스킵. eventId={}", eventId);
                continue;
            }
            ...
            toSave.add(Notification.createPending(userId, NotificationType.RAFFLE_LOSE,
                    "래플 낙첨 알림", "아쉽게도 이번 래플에 당첨되지 않았습니다. 다음 기회를 노려보세요!", raffleId, "RAFFLE"));
        }
