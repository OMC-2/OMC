...
public class CouponIssuedConsumer {
    ...
    public void handle(List<ConsumerRecord<String, String>> records, Acknowledgment acknowledgment) {
        ...
        for (int i = 0; i < records.size(); i++) {
            ...
            try {
                events.add(parseEvent(record.value(), record.topic()));
            } catch (Exception e) {
        ...
        for (Map<String, Object> event : events) {
            ...
            if (duplicates.contains(eventId)) {
                log.debug("[CouponIssuedConsumer] 중복 이벤트 스킵. eventId={}", eventId);
                continue;
            }
            ...
            toSave.add(Notification.createPending(userId, NotificationType.COUPON_ISSUED,
                    "쿠폰 발급 알림", "쿠폰이 발급되었습니다. 결제 시 사용해보세요!", couponId, "COUPON"));
        }
