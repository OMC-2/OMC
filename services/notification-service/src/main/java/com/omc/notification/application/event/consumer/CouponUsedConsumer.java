    ...
    public void handle(List<ConsumerRecord<String, String>> records, Acknowledgment acknowledgment) {
        ...
        for (int i = 0; i < records.size(); i++) {
            ...
            try {
                ...
                log.info("[CouponUsedConsumer] 수신. topic={}, offset={}, eventId={}", record.topic(), record.offset(), event.get("eventId"));
                events.add(event);
            } catch (Exception e) {
        ...
        for (Map<String, Object> event : events) {
            ...
            if (duplicates.contains(eventId)) {
                log.debug("[CouponUsedConsumer] 중복 이벤트 스킵. eventId={}", eventId);
                continue;
            }
            ...
            toSave.add(Notification.createPending(userId, NotificationType.COUPON_USED,
                    "쿠폰 사용 알림", "쿠폰이 사용되었습니다. 할인 혜택이 적용되었어요!", couponId, "COUPON"));
        }
