Feature: 알림 인증·인가 보안
  ...
  #
  # 시나리오 목록:
  #   1. [예외] X-Gateway-Secret 없이 알림 목록 조회 → 403
  #   2. [예외] X-Gateway-Secret 없이 읽음 처리 → 403
  #   3. [예외] 토큰 없이 알림 목록 조회 → 401
  #   4. [예외] 토큰 없이 읽음 처리 → 401
  #   5. [예외] 변조된 JWT로 알림 목록 조회 → 401
  #   6. [정상] USER 토큰으로 알림 목록 조회 → 200
  #
  ...
  # ----------------------------------------------------------------
  # 시나리오 1: X-Gateway-Secret 없이 알림 목록 조회 시 403을 반환한다
  # ----------------------------------------------------------------
  Scenario: [예외] X-Gateway-Secret 없이 알림 목록 조회 → 403
  ...
  # ----------------------------------------------------------------
  # 시나리오 2: X-Gateway-Secret 없이 읽음 처리 시 403을 반환한다
  # ----------------------------------------------------------------
  ...
  # ----------------------------------------------------------------
  # 시나리오 3: 토큰 없이 알림 목록 조회 시 401을 반환한다
  # ----------------------------------------------------------------
  ...
  # ----------------------------------------------------------------
  # 시나리오 5: 변조된 JWT로 알림 목록 조회 시 서명 검증 실패로 401을 반환한다
  # ----------------------------------------------------------------
  ...
  # ----------------------------------------------------------------
  # 시나리오 6: USER 토큰으로 알림 목록 조회 시 200을 반환한다
  # ----------------------------------------------------------------
